...
describe('expandRemoteControlLaunchPrompt', () => {
  ...
  it('stamps thin post-Live brief when connect result is provided', () => {
    ...
    assert.ok(expanded.includes('On wake:'))
    assert.match(expanded, /\*\*Do not\*\* call `poll_connection` to discover/)
    assert.match(expanded, /post \*\*nothing\*\*/)
    assert.match(expanded, /full `owner_message` events \(with `message` body\)/)
    ...
    assert.match(expanded, /`project_scope` pair/)
    assert.match(expanded, /server `instruction` verbatim/)
    ...
    assert.match(expanded, /work_entry_contract.*high session-plan threshold/i)
    assert.match(expanded, /Routine work stays no-plan/i)
    assert.match(expanded, /atomic `advance`/i)
    assert.match(expanded, /projected revision as `expected_revision`/i)
    assert.match(expanded, /explicit `plan_id` \+ `expected_revision`/i)
    assert.match(expanded, /another owner’s plan is read-only/i)
    assert.match(expanded, /manage-plan describe/)
    assert.match(expanded, /manage-plan use/)
    assert.match(expanded, /ordinary global MCP cannot safely bind it/i)
    assert.match(expanded, /Identity comes only from Cursor host state/i)
    assert.match(expanded, /without a native conversation id requires exactly one live attached minted bond/i)
    assert.match(expanded, /refuses siblings/i)
