...
export function buildPostLiveRemoteBrief(opts) {
  ...
  const lines = [
    ...
    '6. When attached, post the **final** answer with `post_session_message({ connection_id, message, phase: "answer", complete_turn: true, agent_name: "Cursor" })`. Omit `complete_turn` on rare mid-turn posts. **Never** post status chrome / connect banners into the session.',
    '7. **Do not re-arm wait.** Leave the background tail running. Host follow keeps writing the wake file after Cursor `turn_ended`. Never run `devspec-remote-wait.mjs --from-end` on a Connect launch.',
    '8. Stop with `devspec.remote-stop` when done.',
    '',
