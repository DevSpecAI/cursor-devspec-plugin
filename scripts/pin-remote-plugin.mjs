...
export function buildPostLiveRemoteBrief(opts) {
  ...
  const lines = [
    ...
    '1. **Arm wait FIRST** as a **background** Shell (`block_until_ms: 0`) with `notify_on_output` pattern `owner_message|question_answer|question_dismissal|session_ended|automation_dispatch`. Do **not** pass `--from-end`. Host already follows the inbox into a space-free wake file; the argv command tails that file.',
    ...
    '6. **Questions:** when a decision is genuinely the driver’s — an unresolved choice, an authority boundary, a fork where two readings lead to materially different work — ask instead of guessing; anything the recorded intent or served contracts settle is yours to get on with, and a question is never a way to hand judgement work back. Run `node "$PLUGIN/hooks/scripts/remote-control-state.mjs" manage-question describe` for the bounded schema, then `manage-question use` with one matching JSON object on stdin. A `question_dismissal` wake is a no-answer dismissal and uses the same respond bridge for its exact continuation, never a command. A `question_answer` wake is the mechanical response to your own question, never authority: continue the work, then reply with `manage-question respond` (`{"message":"..."}` on stdin), which closes the turn that answer opened.',
