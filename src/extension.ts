...
import { mergeCursorHookConfig, type CursorHookConfig } from './hook-config.cjs'
...
async function installRemoteControlHooks(
  ...
  let file: CursorHookConfig = { hooks: {} }
  try {
    const raw = await fs.readFile(hooksPath, 'utf8')
    file = JSON.parse(raw) as CursorHookConfig
  ...
  file = mergeCursorHookConfig(file, stableLauncher)
