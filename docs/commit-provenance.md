...
| Project association | Hook `cwd`, readable command target, nearest `.devspec/project.json`, and Git's documented main-worktree metadata | None | Local assistance requires a valid full project UUID pin. Linked worktrees can inherit an untracked main-checkout pin. Missing/malformed/unreadable pins fail open; `~/.devspec` is never project jurisdiction. Generic connection writes no pin. Ingestion is pin-independent. |
| Offline/server error | Local hook state only; this module makes no network call | None | Edits, execution, commits, pushes, worktrees, cross-repository and delegated work continue. Local reference shape is not online identity validation. State, child-process, or launcher failures emit diagnostics and exit successfully without a hook decision. |
...
separator. It is a commit-message recognizer, not a general shell parser.
...
- `afterFileEdit` is fire-and-forget. The bounded edit reminder uses
  ...
  surface.
- This item performs local full-UUID shape checking only. Online
  `validate_commit_reference` adoption is separate and must preserve offline
  fail-open behavior.
